...
def create_next_candidates(prev_candidates, length):
    ...
    # create combinations(candidate) check if each x is in prev_cand

    prev_candidates = [list(x) for x in prev_candidates]
    # print(prev_candidates)
    p_cand = spark.createDataFrame(prev_candidates)

    next_candidates = []
    l = []
    for x in tmp_next_candidates:
        p = []
        for y in combinations(x,length - 1):
            p.append(list(y))
        temp_cand = spark.createDataFrame(p)
        if temp_cand.count() == p_cand.intersect(temp_cand).count():
